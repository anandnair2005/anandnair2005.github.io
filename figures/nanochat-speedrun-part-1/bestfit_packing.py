...
def build():
    W = 620
    PAD_L, PAD_R = PAD, PAD
    WASTE_LANE = 86    # reserved so the discarded tail has room to sit
    track = W - PAD_L - PAD_R - WASTE_LANE
    ROW_H, STEP_GAP = 26, 22
    y0 = 74
    ...
    H = y0 + len(steps) * (ROW_H + STEP_GAP) + 68
    ...
    f.text(PAD_L, 42, f"row capacity = T + 1 = {ROW_CAPACITY:,} tokens, "
    ...
    for si, (kind, docs, take, caption) in enumerate(steps):
        ...
        for di, d in enumerate(docs):
            ...
            f.rect(x, y, 2, ROW_H, TEXT, r=0)    # the BOS token
            if w > 44:
                f.text(x + w / 2, y + ROW_H / 2 + 4, f"{d:,}", FS_META, "#1a1a1a",
        ...
        if kind == "crop":
            ...
            f.rect(x, y, 2, ROW_H, TEXT, r=0)
            if w > 44:
                f.text(x + w / 2, y + ROW_H / 2 + 4, f"{take:,}", FS_META, "#1a1a1a",
                       anchor="middle", mono=True)
            # the discarded tail, at the same token scale, in its own lane
            wx = x + w + 7
            f.rect(wx, y + 7, CROP_WASTE * px, ROW_H - 14, RED, r=2, opacity=0.4)
            f.text(wx, y + ROW_H + 12, f"{CROP_WASTE:,} discarded", FS_META, RED)

        f.text(PAD_L, y - 7, f"{si + 1}. {caption}", FS_META, MUTED)

    y_end = y0 + len(steps) * (ROW_H + STEP_GAP)
    f.line(PAD_L, y_end + 2, W - PAD_L, y_end + 2, BROWN, 1, opacity=0.4)
    f.o.append(f'<text x="{PAD_L}" y="{y_end + 22}" font-size="{FS_META}" fill="{MUTED}">'
    ...
    f.text(PAD_L, y_end + 40,
           "The cropped document is always the shortest in the buffer, "
           "which minimises what is thrown away at that step.", FS_META, BROWN)
    return f
